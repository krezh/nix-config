{lib, ...}: let
  ...
  in
    ...
    else if !(builtins.pathExists basePath)
    then
      throw ''
...
in {
  ...
  toAttrs = {
    ...
    in
      if matchingExt != null
      then lib.removeSuffix matchingExt filename
      else filename;
    ...
    toAttr = nixPath: let
      ...
      name =
        if useBaseName
        then removeExtension (baseNameOf nixPath)
        else removeExtension (builtins.replaceStrings ["/"] ["_"] relPath);
    in {
      inherit name;
