{
  flake.modules.nixos.wireplumber = {
    ...
    extractDeviceName = nodeName: let
    ...
    groupByDevice = items: extractFn:
      builtins.foldl' (
        acc: item: let
          device = extractDeviceName (extractFn item);
        in
          if device != null
          then acc // {${device} = (acc.${device} or []) ++ [item];}
          else acc
      ) {}
      items;
    ...
    generateDeviceConfig = device: let
      settings =
        cfg.deviceSettings.${
          device
        } or {
